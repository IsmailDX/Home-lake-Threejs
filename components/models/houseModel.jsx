...
export function HouseModel(props) {
  const { nodes, materials } = useGLTF("models/house_on_lake/scene.gltf");
  const modelRef = useRef();
  const scroll = useScroll();

  useFrame((state, delta) => {
    const scrollOffset = scroll.offset;
    let targetPosition = [-0.4, -0.2, -1.5]; // Default position
    let targetRotation = [0, 0, 0]; // Default rotation

    // Page 1
    // scrollOffset < 1 / number of pages
    if (scrollOffset < 1 / 3) {
      targetPosition = [-0.4, -0.2, -1.5 + scrollOffset * 2];
      targetRotation = [0, scrollOffset * Math.PI * 0, 0];
    }
    // Page 2
    else if (scrollOffset < 2 / 3) {
      targetPosition = [-0.4, -0.2, 1.5 + scrollOffset * 2];
      //the number after the Math.PI is the number for the rotation
      targetRotation = [0, scrollOffset * Math.PI * 0.3, 0];
    }
    // Page 3
    else {
      targetPosition = [1.2, -0.1, 3.3 + scrollOffset * 1];
      targetRotation = [0, 10.4, 0];
    }
...
useGLTF.preload("models/house_on_lake/scene.gltf");
